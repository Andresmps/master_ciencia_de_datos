...
def pedir_datos(nombre_atributo: str, etiqueta_atributo: str, tabla: str, **kwargs):
    ...
    if kwargs.get(nombre_atributo) is None:
        while True:
            ...
            if dato == ".":
                break
            # Avoid empty strings and repeated values to avoid errors in Cassandra
            elif dato in datos or "".strip() == dato:
                logging.info(
                    f"El/La {etiqueta_atributo} ya está en la lista de {nombre_atributo} o es un string vacio, inténtelo de nuevo.")
                continue
            else:
                datos.append(dato)
    else:
